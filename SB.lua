...
local Section = Tab:CreateSection("Bot")
local Toggle = Tab:CreateToggle({
   Name = "Combat Bot",
   CurrentValue = false,
   Flag = "Toggle1", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
                    ...
                    spawn(function()
                        while wait() do
                            ...
                            local function getclosest()
                              ...
                              local cuhh = 42069699 --leyio makes me wanna
                              local ong = nil
                              for i,v in pairs(game.Players:GetPlayers()) do
                                  pcall(function()
                                  if (hrp.Position - v.Character.HumanoidRootPart.Position).Magnitude < cuhh then
                                      if v.Name ~= char.Name and not v.Character:FindFirstChild("rock") then
                                          cuhh = (hrp.Position - v.Character.HumanoidRootPart.Position).Magnitude
                                          ong = v.Name
                                      end
                                  end
                                  end)
                            ...
                            local function got(playr)
                              getchar()
                              local BALLS = game.Workspace:FindFirstChild(playr).HumanoidRootPart.Position
                              char.Humanoid:MoveTo(BALLS)
                            end
    ...
    for _, Player in ipairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer then
            ...
            if Character then
                ...
                if Humanoid then
                    if Humanoid.Health > 0 then -- check if player is alive
                        ...
                        if HumanoidRootPart then
                            ...
                            Hitbox.LineThickness = 0.01
                            Hitbox.Transparency = 0.5
                            Hitbox.Parent = Character
                            Hitbox.Parent = workspace
                            HumanoidRootPart.Size = HumanoidRootPart.Size + Vector3.new(25, 25, 25)
                        end
                    else
                        -- add a delay and reapply the effect when player respawns
                        spawn(function()
...
local function onPlayerAdded(Player)
    Player.CharacterAdded:Connect(function(Character)
        ...
        if HumanoidRootPart then
            local Hitbox = Instance.new("SelectionBox")
            Hitbox.Adornee = HumanoidRootPart
            Hitbox.Color3 = Color3.fromRGB(50, 255, 135)
            Hitbox.LineThickness = 0.01
            Hitbox.Transparency = 0.5
            Hitbox.Parent = Character
            Hitbox.Parent = workspace
